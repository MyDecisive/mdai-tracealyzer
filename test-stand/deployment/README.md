...
- `datadog-agent.yaml` — DaemonSet running the official Datadog agent in APM-only mode, plus its ServiceAccount and a `datadog-agent-$(DEMO_NAMESPACE)` ClusterRole/ClusterRoleBinding granting read access to `nodes`, `namespaces`, `pods`, `endpoints`, and `endpointslices` (needed for kubelet metadata enrichment). Forwards APM (`DD_APM_DD_URL`) to `DEMO_DATADOG_FORWARDER_URL`, substituted at apply time by the Makefile. Mounts `/opt/datadog-agent/run` for pointer-file state.
