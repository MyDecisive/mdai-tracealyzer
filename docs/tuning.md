...
| `topology_emissions_failed_total` rising | Raise `max_retries` and `initial_backoff` before adjusting `batch_size`. Batch size does not address transient errors; it only changes how many rows each failure drops. |
